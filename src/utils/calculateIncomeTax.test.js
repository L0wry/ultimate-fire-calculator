...
describe('Calculate Tax', () => {
  ...
  it('calculates medium band correctly with the default tax free amount', () => {
    ...
    expect(calculateIncomeTax(incomeTaxBands(), taxableIncome).mediumBand).toStrictEqual({
      carryOver: 0, end: 150000, start: 50000.01, taxPaid: 11020, taxPercent: 0.4
    });
  });
  ...
  it('calculates upper band correctly', () => {
    ...
    expect(calculateIncomeTax(incomeTaxBands(), taxableIncome).upperBand).toStrictEqual({
      carryOver: 0, end: 99999999, start: 150000.01, taxPaid: 28125.01, taxPercent: 0.45
    });
  });
  ...
  it('Matches the snapshot', () => {
    ...
    expect(calculateIncomeTax(incomeTaxBands(), taxableIncome)).toStrictEqual({
      lowerBand: {
        carryOver: 162500.01, end: 50000, start: 12500.01, taxPaid: 7500, taxPercent: 0.2
      },
      mediumBand: {
        carryOver: 62500.02000000002, end: 150000, start: 50000.01, taxPaid: 40000, taxPercent: 0.4
      },
      taxFreePersonalAllowance: 0,
      taxFreePersonalAllowanceRemovedBy100kTax: 12500,
      totalIncomeTax: 75625.01,
      upperBand: {
        carryOver: 0, end: 99999999, start: 150000.01, taxPaid: 28125.01, taxPercent: 0.45
      }
    });
